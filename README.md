...
5. 如果报错 `Could not create named generator Visual Studio 18 2026`：说明你本机没有这个生成器，请改用你机器里实际可用的（你截图里是 `Visual Studio 17 2022`）。

### Step H：先查你机器支持哪些生成器（推荐）
```powershell
cmake --help
```
在输出的 `Generators` 列表里找带 `*` 或可用的 Visual Studio 项，再把 `-G` 参数改成对应名称。
